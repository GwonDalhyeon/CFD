...
bool CircumCircle(const Vector2D<double> & p, const Vector2D<double> & p1, const Vector2D<double> & p2, const Vector2D<double> & p3, Vector2D<double> & center, double &r)
{
...
void DelaunayTriangulate(const VectorND<Vector2D<double>> & points)
{
	int triMax = points.iLength * 4;
	int edgeMax = 100;
	
	VectorND<Vector2D<int>> Edge(1,edgeMax);
	VectorND<Vector2D<int>> tempEdge;
...
#pragma omp parallel for
	...
	VectorND<bool> complete(points.iStart, points.iLength);
	...
	*/
	...
	the triangle list.
	*/
	VectorND<Vector2D<double>>  newPoints(points.iStart, points.iLength + 1);
#pragma omp parallel for
	for (int i = points.iStart; i <= points.iEnd; i++)
	{
	...
	for (int i = points.iStart; i <= points.iEnd; i++)
	{
		...
		*/
		for (int j = 1; j <= numTri; j++)
		{
			...
			inside = CircumCircle(currentPoint, P1, P2, P3, center, radius);

			// Suggested
			// if (xc + r + EPSILON < xp)
			if (center.x + radius<currentPoint.x)
			{
				complete(j) = true;
			}
			...
			if (inside)
			{
				...
				MATLAB.Variable("nbhdEdge", nbhdEdge);
				Edge(nbhdEdge).Variable("edge");
	...
	*/
	...
	VectorND<Polygon2D> Triangles(1, numTri);
//#pragma omp parallel for
	for (int i = 1; i <= numTri; i++)
	{
		Triangles(i) = Polygon2D(3);
		Triangles(i).VerticesIndex = triangle(i);
		Triangles(i).Vertices(1) = points(triangle(i)(1));
		Triangles(i).Vertices(2) = points(triangle(i)(2));
		Triangles(i).Vertices(3) = points(triangle(i)(3));
		VecND2DVariable("tri", Triangles(i).Vertices);
		MATLAB.Command("plot([tri(:,1); tri(1,1)],[tri(:,2); tri(1,2)])");
	}
